...
const chatbotResponses = [
    ...
    },
    {
        ...
        response: "Marc's security philosophy is 'practical security that enables the mission.' He believes:\n• Security should facilitate business objectives, not block them\n• Perfect security doesn't exist - risk management is key\n• Documentation and communication are as important as technical controls\n• Compliance frameworks are tools, not goals\n• People, process, and technology must work together\n\nHe focuses on measurable improvements over checkbox compliance."
    },
...
// ============================================
        ...
        function analyzeJobFit(jobDesc) {
            ...
            // ===== SKILL WEIGHTS CONFIGURATION =====
            // Assign importance weights to different skills
            const skillWeights = {
                // CRITICAL SKILLS (10 points)
                "nist 800-53": 10, "nist": 10, "800-53": 10,
                "rmf": 10, "risk management framework": 10, "fisma": 10,
                "cissp": 10, "security clearance": 10, "secret clearance": 10,
                
                // CORE TECHNICAL (8 points)
                "aws": 8, "amazon web services": 8, "azure": 8,
                "cloud security": 8, "grc": 8, "governance": 8,
                "compliance": 8, "iso 27001": 8, "soc 2": 8, "soc2": 8,
                
                // IMPORTANT TECHNICAL (6 points)
                "python": 6, "powershell": 6, "sql server": 6, "sql": 6,
                "vulnerability management": 6, "penetration testing": 6,
                "incident response": 6, "security assessment": 6,
                "security audit": 6, "ceh": 6,
                
                // SUPPORTING SKILLS (4 points)
                "windows server": 4, "active directory": 4,
                "vmware": 4, "hyper-v": 4, "virtualization": 4,
                "scap": 4, "nessus": 4, "technical writing": 4,
                
                // SOFT SKILLS (2 points)
                "leadership": 2, "project management": 2, "communication": 2
            };
            ...
            for (const [skill, weight] of Object.entries(skillWeights)) {
                if (jobLower.includes(skill)) {
                    ...
                    const hasSkill = allSkills.some(candidateSkill => 
                    ...
                    if (hasSkill) {
                        ...
                        if (weight >= 10) {
                            criticalMatches.push(skill);
                        } else if (weight >= 6) {
                            coreMatches.push(skill);
                        }
                    }
                }
            }
            
            // Catch additional unweighted matches
            let additionalMatches = 0;
            allSkills.forEach(skill => {
                if (jobLower.includes(skill) && !matchedSkills.some(m => m.skill === skill)) {
                    additionalMatches++;
                    matchedSkills.push({ skill, weight: 1 });
                }
            });
            
            earnedPoints += additionalMatches;
            totalPossiblePoints += additionalMatches;
            ...
            if (yearsRequired) {
                const requiredYears = parseInt(yearsRequired[1]);
                if (candidateYears >= requiredYears) {
                    experienceBonus = Math.min(10, (candidateYears - requiredYears) * 2);
                }
            }
            ...
            if (requiresLeadership && CANDIDATE_INFO.skills["Leadership & Management"]) {
                leadershipBonus = 5;
            }
            
            // ===== CALCULATE SCORE =====
            let baseScore = totalPossiblePoints > 0 
                ? Math.round((earnedPoints / totalPossiblePoints) * 70) 
                : 0;
            
            let score = Math.min(95, baseScore + experienceBonus + leadershipBonus);
            ...
            if (score >= 75) {
                level = 'high';
                summary = `${CANDIDATE_INFO.name} is an excellent match for this role with strong alignment across critical skills and experience.`;
                
                if (criticalMatches.length > 0) {
                    strengths.push(`${criticalMatches.length} critical skill match(es): ${criticalMatches.slice(0, 3).join(', ')}`);
                }
                if (coreMatches.length > 0) {
                    strengths.push(`${coreMatches.length} core technical skill(s) matched`);
                }
                strengths.push(`${CANDIDATE_INFO.about.yearsExperience} years of relevant experience${yearsRequired ? ` (exceeds ${yearsRequired[1]}+ year requirement)` : ''}`);
                strengths.push(`${matchedSkills.length} total matching skills identified`);
                
                if (requiresLeadership) {
                    strengths.push("Demonstrated leadership and team management experience");
                }
                
                recommendation = `${CANDIDATE_INFO.name} is a strong candidate for this position. High skill alignment and relevant experience. Recommend moving forward with an interview.`;
                
            } else if (score >= 50) {
                level = 'medium';
                summary = `${CANDIDATE_INFO.name} has relevant experience for this role with ${missingCritical.length > 0 ? 'some critical' : 'minor'} skill gaps that could be addressed.`;
                
                if (criticalMatches.length > 0) {
                    strengths.push(`Critical skills matched: ${criticalMatches.join(', ')}`);
                }
                if (coreMatches.length > 0) {
                    strengths.push(`${coreMatches.length} core technical skills aligned`);
                }
                strengths.push(`${CANDIDATE_INFO.about.yearsExperience} years of professional experience`);
                strengths.push(`Strong foundational knowledge in related technologies`);
                
                if (missingCritical.length > 0) {
                    gaps.push(`Critical skills to develop: ${missingCritical.slice(0, 3).join(', ')}`);
                }
                if (missingCore.length > 0 && missingCritical.length === 0) {
                    gaps.push(`Some specific technologies not listed: ${missingCore.slice(0, 3).join(', ')}`);
                }
                
                recommendation = `${CANDIDATE_INFO.name} could be a good fit with some training or ramp-up time. Worth considering for interview to assess transferable skills.`;
                
            } else {
                level = 'low';
                summary = `While ${CANDIDATE_INFO.name} has strong general technical skills, there is limited direct overlap with this specific role's requirements.`;
                
                strengths = [
                    `Strong technical fundamentals in cybersecurity and GRC`,
                    `${CANDIDATE_INFO.about.yearsExperience} years of professional experience`,
                    "Proven ability to learn new technologies"
                ];
                
                if (matchedSkills.length > 0) {
                    strengths.push(`Some relevant skills: ${matchedSkills.slice(0, 3).map(m => m.skill).join(', ')}`);
                }
                
                if (missingCritical.length > 0) {
                    gaps.push(`Limited experience with critical requirements: ${missingCritical.slice(0, 3).join(', ')}`);
                }
                gaps.push("Different primary focus area than role requirements");
                
                recommendation = `Consider ${CANDIDATE_INFO.name} if the role has flexibility or if transferable skills are valued. May require significant ramp-up time.`;
            }
        ...
        function displayFitResult(analysis) {
            ...
            fitResult.innerHTML = `
                ...
                <div style="margin-top: 1.5rem;">
                    <strong style="color: var(--primary); display: block; margin-bottom: 0.5rem;">✓ Key Strengths:</strong>
                ...
                <div style="margin-top: 1rem;">
                    <strong style="color: var(--accent); display: block; margin-bottom: 0.5rem;">⚠ Areas to Consider:</strong>
                    <ul style="margin: 0; padding-left: 1.5rem; color: var(--gray);">
